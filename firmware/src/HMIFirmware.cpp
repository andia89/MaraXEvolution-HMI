...
extern const uint8_t profile_editor_start[] asm("_binary_src_www_profile_html_start");
extern const uint8_t profile_editor_end[] asm("_binary_src_www_profile_html_end");
...
void handleProfileWebIndex()
{
  ...
  size_t html_size = profile_editor_end - profile_editor_start;
  server.sendHeader("Cache-Control", "no-cache, no-store, must-revalidate");
  server.sendHeader("Pragma", "no-cache");
  server.sendHeader("Expires", "-1");
  server.setContentLength(html_size);
  server.send(200, "text/html", "");
  server.sendContent((const char *)profile_editor_start, html_size);
  server.client().stop();
}
